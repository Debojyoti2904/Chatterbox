...
def router_node(state: AgentState, config: RunnableConfig) -> AgentState:
    ...
    try:
        ...
        print(f"Router LLM Error: {e}. Defaulting to RAG.")
        result = RouteDecision(route="rag")
...
def rag_node(state: AgentState, config: RunnableConfig) -> AgentState:
    ...
    judge_messages = [
        ("system", (
            ...
            "Respond ONLY with the structured output object."
        )),
        ("user", f"Question: {query}\n\nRetrieved context: {chunks}")
    ]
    
    # --- FIX: Wrapped in try-except to prevent 400 Bad Request ---
    try:
        verdict = judge_llm.invoke(judge_messages)
        is_sufficient = verdict.sufficient
    except Exception as e:
        print(f"!!! LLM Structured Output Failed: {e}")
        print("Defaulting to 'False' (Not Sufficient) to prevent crash.")
        is_sufficient = False 
    # -----------------------------------------------------------

    print(f"RAG Judge verdict: {is_sufficient}")
...
Provide a helpful, accurate, and concise response based on the available information."""
    ...
    try:
        ...
        ans = "I'm sorry, I encountered an error while generating the final answer."
